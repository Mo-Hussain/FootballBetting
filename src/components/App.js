...
export class App extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			bets: {
				// TODO: rename confusing array names below
				nonUserBets: [],
				userBets: [],
				userAcceptedBets: []
			}
	...
	componentDidMount() {
		...
		getWeb3()
			...
			.then(contractInstance => {
				...
				let getBets = function () {
					console.log("BetChange event triggered.");
					getAllBets(contractInstance, this.state.ethereum.web3, this.state.ethereum.currentAccount, this.state.bets)
					.then(bets => {
						console.log(bets);		
						let updatedState = this.state;
				...
				// TODO: fix BetChange event being called twice
				contractInstance.BetChange().watch(getBets);

				return getAllBets(contractInstance, this.state.ethereum.web3, this.state.ethereum.currentAccount, this.state.bets);
			})
			.then(bets => {
				console.log(bets);
				let updatedState = this.state;
		...
		return (
			<div>
				...
				<Switch>
					...
					<Route path="/bets">
						{/* TODO: fix footer to pass filled_bets to BetsPage component */}
						...
						<BetsPage unfilled_bets={this.state.bets.userAcceptedBets} ethereum={this.state.ethereum} />
					</Route>
				...
				<Footer filled_bets={this.state.bets.userBets} ethereum={this.state.ethereum} />
			</div>
		);
	}
}
